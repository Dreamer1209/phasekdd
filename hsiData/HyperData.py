...
class NTIRE2022(torch.utils.data.Dataset):
    ...
    def loadCube(self, cube_path):
        ...
        with h5py.File(cube_path, 'r') as f:
            ...
            cube[cube<0] = 0
            cube = (cube - cube.min()) / (cube.max() - cube.min() + 1e-8)  # normalize
    ...
    def loadData(self, img_path, cube_path):
        ...
        if self.load_img_type == 'rgb':
            img = loadBGR(img_path)
        elif self.load_img_type == 'gray':
            img = loadGray(img_path)
        elif self.load_img_type == 'hsv':
            img = loadHSV(img_path)
        # print(f"NTIRE Original RGB shape: {img.shape}, value range: ({img.min()}, {img.max()})")
        # Normalize RGB image
        img = (img - img.min()) / (img.max() - img.min())
        # print(f"NTIRE Normalized RGB shape: {img.shape}, value range: ({img.min()}, {img.max()})")
        # Load hyperspectral image
        cube = self.loadCube(cube_path)
        # print(f"NTIRE Original Cube shape: {cube.shape}, value range: ({cube.min()}, {cube.max()})")

        return img, cube

    def __getitem__(self, idx):
        # img_idx = idx // self.patch_per_img
        img, cube = self.loadData(self.img_files[idx], self.cube_files[idx])
        ...
        if self.do_aug:
            ...
            img = self.arguement(img, rotTimes, vFlip, hFlip)
            cube = self.arguement(cube, rotTimes, vFlip, hFlip)
        
        if self.transform is not None:
            all = np.concatenate([img, cube], axis = -1)
